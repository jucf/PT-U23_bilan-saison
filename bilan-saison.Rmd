...
# "https://intervals.icu/api/athlete/i38318/activities.csv"
# i si profil créer sur intervals et sinon jsute les chiffres si créer avec strava
# nom athlète
athlete <- "Francis Juneau"
# data
metriques_csv <- "i38318_activities.csv"
records_csv <- "athlete_i38318_power_curves.csv"
# selection période
selection_debut <- "2021-01-01"
selection_fin <- "2021-12-17" 
# déoucpage période
intervalle <- "1 month"
...
metriques_clean <- df_2 %>%
   ...
   mutate(
      ...
      fraicheur_kj = round((chronique_42_kj - aigue_7_kj) / 100, digits = 1),
      aigue_7_kj_sd = roll_sd(icu_joules_divised, 7, min_obs = 1),
      monotonie_kj = round(aigue_7_kj / aigue_7_kj_sd, digits = 1),
   ...
   mutate(
      monotonie_kj = ifelse(monotonie_kj >= 10, NA, monotonie_kj)
   )
...
records_clean <- records %>% 
   janitor::clean_names() %>% 
   mutate(
     this_season_wkg = round(this_season / poids_kg$icu_weight, digits = 1),
      this_season_2000k_j_wkg = round(this_season_2000k_j / poids_kg$icu_weight, digits = 1),
      this_season_3000k_j_wkg = round(this_season_3000k_j / poids_kg$icu_weight, digits = 1)
   )
```

**Comparaison de différentes méthodes de quantification de la charge d'entraînement**

```{r TL, fig.cap="Méthodes de quantification de la charge d'entraînement externe (Watts, travail) et interne (Fc). \\newline{}\\textbf{Charge d'entraînement chronique}: représente la charge globale que votre corps est habitué à supporter (derniers 42 jours). En général, vous pouvez voir la charge chronique comme un indicateur du niveau de forme physique: plus votre corps peut s'entraîner, plus vous êtes en forme. \\newline{}\\textbf{Charge d'entraînement aigue}: représente votre charge d'entraînement au cours de la dernière semaine (derniers 7 jours). En particulier, la charge d'entraînement aigue est liée à la fatigue: l'augmentation de la fréquence des entraînements intenses au cours des derniers jours entraîne une fatigue plus élevée.", fig.width=11, fig.height=12}
gg_tss <-
...
gg_kj <-
   ...
   scale_fill_manual(name = "",
                    ...
                    labels = "Données journalières",
                    drop = FALSE) +
   labs(y = "Kilojoules simplifié (kJ)\n", x = "") +
   ggtitle("Charge: travail simplifiée (kJ)") +
   theme_minimal() +
   theme(
      legend.position = "top",
      legend.title = element_blank(),
...
gg_trimp <-
   ...
   ggtitle("Charge: Lucia's training impulse (fréquence cardique)") +
   theme_minimal() +
   theme(
      legend.position = "top",
      legend.title = element_blank(),
...
data_cp <- records_clean %>% 
   select(secs, this_season) %>% 
   rename(
      TTE = secs,
      PO = this_season
   ) %>% 
   na.omit() %>% 
   filter(TTE %in% c(120, 180, 300, 900, 1200))
...
Puissance critique (CP): **`r round(cp)`**
$\color{Gray}{\textbf{ | }}$
W': **`r W`**

```{r CP, fig.cap="Modèle avec le meilleure combinaison.", results = 'hide', fig.width=11, fig.height=4,}
gg <- combinations_results %>% 
  slice_min(RMSE, n = 1) %>% 
  dplyr::pull(plot)
...
Zones d'entraînement basées sur une puissance critique de **`r round(cp)`** Watts
...
df_z_p %>%
   ...
   footnote(
      general_title = "",
      general = "Les zones puissane sont basées sur le modèle de Coggan.",
      threeparttable = T,
      fixed_small_size = T
   )
```

Zones d'entraînement basées sur une fréquence cardiaque maximale de **`r fc_max`** bpm
...
# ajouter unité
df_z_fc$"Fréquence cardiaque" <- paste0(df_z_fc$"Fréquence cardiaque", " bpm")
...
df_z_fc %>%
   ...
   footnote(
      general_title = "",
      general = "Les zones de fréquence cardiaque sont basées sur le modèle de Stephen Seiler.",
      threeparttable = T,
      fixed_small_size = T
   )
```
...
**Distribution mensuel du volume d'entraînement**

```{r heuresTID, fig.cap="Heures totales et distribution de l'intensité d'entraînement selon le modèle de 3 zones de puissance (voir tableau des zones d'entraînement pour référence). En sport d'endurance, il est recommandé de limiter le temps d'entraînement en zone 2 pour la majorité de la planification annuelle. Une distribution de l'intensité de type «polarisé» ≈ 80\\% zone 1 et ≈ 20\\% zone 3 semble être optimale pour l'amélioration des performances. Note: il peut y avoir une différence entre les heures totales et le temps par zones si certains entraînements ont été effectués sans capteur de puissance.", fig.width=11, fig.height=12}
...
gg_tid_pct <- metriques_clean %>%
   ...
   coord_cartesian(ylim = c(min(0), max(100)),
                   expand = TRUE,
                   default = TRUE) +
   ggtitle("Pourcentages par zones") +
   theme_minimal() +
   theme(
      legend.position = "top",
      legend.title = element_blank(),
...
**Résumé de la saison en chiffre selon 4 grandes périodes**
...
gg_kj_lutrimp <- metriques_clean %>%
   ...
   geom_line(
      aes(y = ACWR_kj_ravg,
          color = "Travail - kJ (chronique/aigue) "),
      size = 1.25
   ) +
   geom_line(
      aes(y = ACWR_lutrimp_ravg,
          color = "LuTRIMP - Fc (chronique/aigue)"),
      size = 1.25
   ) +
   ...
   coord_cartesian(ylim = c(min(0), max(2.5)),
                   expand = TRUE,
                   default = TRUE) +
...
```{r perf, fig.cap="Analyse des variations d'état d'entraînement. La relation entre les variations de charge d'entrainement, la distribution de l'intenstié et les marqueurs d'état d'entraînement et de performance permet de mieux comprendre les conditions favorables et défavorables à la performance. \\newline{}\\textbf{Efficacité aérobie}: relation entre la puissance et la fréquence cardiaque. Intuitivement, une fréquence cardiaque plus basse pour la même puissance se traduit par une meilleure endurance aérobie. De même, une puissance plus élevée à la même fréquence cardiaque est lié à une meilleure endurance aérobie. \\newline{}\\textbf{Estimation CP et W'}: caclulées à patir d'un effort maximal compris entre 180 secondes et 30 minutes mis en relation avec votre profil de record de puissance.", fig.width=11, fig.height=11.5}
gg_tid_pct <- metriques_clean %>%
   select(date, zone1, zone2, zone3) %>%
   group_by(mois = cut(date, "month")) %>%
   summarise_at(vars(zone1, zone2, zone3),  sum, na.rm = TRUE) %>%
   ...
   coord_cartesian(ylim = c(min(0), max(100)),
                   expand = TRUE,
                   default = TRUE) +
   ggtitle("Distribution intenstité (zones puissance)") +
   theme_minimal() +
   theme(
      legend.position = "top",
      legend.title = element_blank(),
...
df_full <- full_join(df,e_perf) %>% 
   ...
   # remplacer NA par dernière valeur pour évuter drop data
   na.locf(na.rm = FALSE)
...
mmp <- records_clean %>%
   ...
   mutate(
      records = fct_recode(
         records,
         "Records saison" = "this_season_wkg",
         "Records après 2000 kJ" = "this_season_2000k_j_wkg",
         "Records après 3000 kJ" = "this_season_3000k_j_wkg"
      ),
      records = factor(
         records,
...
gg_mmp <- mmp %>%
   ...
   scale_y_continuous(breaks = seq(
      from = 0,
      to = max(mmp$watts_kg + 1),
      by = 1
   )) +
   coord_cartesian(ylim = c(min(0), max(mmp$watts_kg + 1)),
                   expand = TRUE,
                   default = TRUE) +
...
tab_mmp <- records_clean %>%
   select(secs, this_season:this_season_3000k_j, this_season_wkg:this_season_3000k_j_wkg) %>%
   ...
   rename(
         "Records saison (W)" = "this_season",
         "Records après 2000 kJ (W)" = "this_season_2000k_j",
         "Records après 3000 kJ (W)" = "this_season_3000k_j",
         "Records saison (W/kg)" = "this_season_wkg",
         "Records après 2000 kJ (W/kg)" = "this_season_2000k_j_wkg",
         "Records après 3000 kJ (W/kg)" = "this_season_3000k_j_wkg"
   ) %>% 
...
```{r coggan, fig.cap="Charte de record de puissance de Coggan en fonction du niveau de performance. Permet une appréciation générale des forces et faiblesses."}
...
```{r corr, fig.cap="Matrice de corrélation. Le coefficient de corrélation R (1 à -1) indique la force de la corrélation et le sens de celle-ci. Lorsque le R est positif (vert), les valeurs des deux variables tendent à augmenter ensemble tandis que lorsque le R est négatif (rouge), les valeurs d’une variable tendent à augmenter lorsque les valeurs de l’autre variable diminuent. Les valeurs R de 1 et -1 représentent chacune des corrélations «parfaites», positive et négative. L'intensité de la couleur et la circonférence des cercles indiquent la valeur de la corrélation R. La probabilité statistique (valeur p) et son niveau de signification est représenté par les symboles \\textbf{*} (0.001 = \\textbf{***}, 0.01 = \\textbf{**}, 0.05 = \\textbf{*}).", fig.width=11, fig.height=12.5}
