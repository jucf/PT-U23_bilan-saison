...
# "https://intervals.icu/api/athlete/5640970/activities.csv"
# i si profil créer sur intervals et sinon jsute les chiffres si créer avec strava
# nom athlète
athlete <- "David Barry"
# data
metriques_csv <- "5640970_activities.csv"
records_csv <- "athlete_5640970_power_curves.csv"
# selection période
selection_debut <- "2021-01-01"
selection_fin <- "2021-12-22" 
# déoucpage période
intervalle <- "1 month"
...
```{r CP, fig.cap="Représentation graphique de la modélisation.", results = 'hide'}
gg <- simple_results %>% 
   filter(method == "1/time") %>% 
   dplyr::pull(plot)
...
gg_mmp <- mmp %>%
   ...
   scale_y_continuous(breaks = seq(
      from = 0,
      to = max(mmp$watts_kg, na.rm = TRUE),
      by = 1
   )) +
   coord_cartesian(ylim = c(min(0), max(mmp$watts_kg,  na.rm = TRUE)),
                   expand = TRUE,
                   default = TRUE) +
...
```{r coggan, fig.cap="Charte de record de puissance de Coggan en fonction du niveau de performance. Permet une appréciation générale des forces et des faiblesses."}
